...
class Cartapp:
    ...
    def bill_updates(self):
        try:
            ...
            query = "SELECT COUNT(*) FROM customer WHERE cname = %s AND bill_date BETWEEN %s AND %s"
            self.cursor.execute(query, (customer_name, thirty_days_ago, today))
    ...
    def generate_bill(self):
        if len(self.cart_list) == 0:
            ...
            try:
                ...
                customer_data = (str(self.invoice), time.strftime("%Y-%m-%d"), self.var_cname.get(), self.var_contact.get())
 ...
 Thank You !!!
    ...
    def print_bill(self):
        ...
        if self.chk_print == 1:
            try:
                self.generate_bill()
                pdf_filename = os.path.join("C:\\bills", f"{str(self.invoice)}.pdf")
                doc = SimpleDocTemplate(pdf_filename, pagesize=A4)
                styles = getSampleStyleSheet()
                content = []
                bill_style = ParagraphStyle(name='BillContent', fontName='Helvetica', fontSize=14, leading=16,alignment=1)
                bill_text_lines = self.txt_bill_area.get('1.0', END).split('\n')
                for line in bill_text_lines:
                    content.append(Paragraph(line.strip(), bill_style))
                doc.build(content)
                os.startfile(pdf_filename, 'print')
            except Exception as ex:
                messagebox.showerror('Print Error', f"An error occurred while printing: {str(ex)}", parent=self.root)
        else:
